...
def draw_setup(start_frame): #draws window with 
...
def setup(): #Sets up 
    ...
    draw_setup(start_frame)
    print("testend")
    
def get_random_pop_index(frames_list): #chooses random frame to split if the frame is larger than the minimum area and returns the index of said frame
    global min_area
    poppable = False
    while poppable == False:
        boxpop_index = int(random.randint(0,len(frames_list)-1))
        random_frame = frames_list[boxpop_index]
